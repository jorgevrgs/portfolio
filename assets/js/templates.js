...
var getTitle = project => ({
  tag: 'h3',
  className: ['ma-0', 'title'],
  textContent: project.title
});

var getDescription = project => ({
  tag: 'p',
...
export var projectTemplate = project => {
  var result = {
    tag: 'article',
    className: 'project',
    dataset: {
    ...
    children: [getMedia(project), {
      tag: 'div',
      className: ['d-flex', 'flex-column', 'pa-20', 'bg-primary', 'project-content'],
      children: [getTitle(project), getTags(project), getProjectButtons(project)]
    }]
  };
  return result;
};
export var modalTemplate = project => ({
  tag: 'div',
  className: 'modal',
  dataset: {
    id: project.id
  ...
  children: [{
    tag: 'article',
    className: 'project',
    children: [getButtonClose(), getTitle(project), getTags(project), getMedia(project), {
      tag: 'div',
      className: ['d-flex', 'flex-column', 'pa-20', 'bg-primary', 'project-content'],
